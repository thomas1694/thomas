...
public class Admin_MemberTest {
			...
			private static void list(Admin_MemberDAO dao) {
				...
				List<Admin_MemberDTO> list;
				try {
					list = dao.list(map);
					
					for (int i=0; i<list.size(); i++) {
						Admin_MemberDTO dto = list.get(i);
						p(dto);
						p("------------------------------");
					}	
				} catch (Exception e) {
			...
			private static void update(Admin_MemberDAO dao) {
				Admin_MemberDTO dto = new Admin_MemberDTO();
				dto.setC_id("c_user1");
			...
			private static void p(Admin_MemberDTO dto) {
